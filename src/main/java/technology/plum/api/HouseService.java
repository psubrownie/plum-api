...
import technology.plum.api.beans.LogicalLoadMetric;
import technology.plum.api.beans.Room;
import technology.plum.api.beans.Scene;
...
public class HouseService {
	...
	public static House getHouse(PlumContext ctx, String hid) throws Exception {
		...
		house.setScenes(getScenes(ctx, hid));
		return house;
	}

	private static List<Scene> getScenes(PlumContext ctx, String hid) throws Exception {
		String sidsString = plumPostProd(ctx, "/v2/getScenes", "{\"hid\":\"" + hid + "\"}");
		Type listType = new TypeToken<List<String>>() {
		}.getType();
		List<String> sids = new Gson().fromJson(sidsString, listType);
		List<Scene> scenes = new ArrayList<>();
		for (String sid : sids) {
			scenes.add(getScene(ctx, sid));
		}
		return scenes;
	}

	private static Scene getScene(PlumContext ctx, String sid) throws Exception {
		String resp = plumPostProd(ctx, "/v2/getScene", "{\"sid\":\"" + sid + "\"}");
		Scene s = new Gson().fromJson(resp, Scene.class);
		return s;
	}
	...
	public static LogicalLoadMetric getLogicalLoadMetrics(PlumContext ctx, String llid) throws Exception {
		String resp = plumPostProd(ctx, "/v2/getLogicalLoadMetrics", "{\"llid\":\"" + llid + "\"}");
		LogicalLoadMetric llMetric = new Gson().fromJson(resp, LogicalLoadMetric.class);
		return llMetric;
	}
	...
	public static void setLevel(PlumContext ctx, String llid, Integer level) throws Exception {
		plumPostProd(ctx, "/v2/setLogicalLoadLevel", "{\"llid\":\"" + llid + "\",\"level\":" + level.toString() + "}");
	}
	...
	private static String plumPostProd(PlumContext ctx, String uri, String body) throws Exception {
		...
		httpPost.addHeader("Authorization", "Basic " + ctx.getAuthorizationString());
		if (body != null) {
